...
/// GI ///
	...
	if (bool(instances.data[instance_index].flags & INSTANCE_FLAGS_USE_LIGHTMAP_CAPTURE)) { //has lightmap capture
		...
		if (uses_sh) {
			...
			lm_light_l0 = sample_lightmap_array(lightmap_textures[ofs], uvw + vec3(0.0, 0.0, 0.0), lightmaps.data[ofs].light_texture_size).rgb;
			lm_light_l1n1 = (sample_lightmap_array(lightmap_textures[ofs], uvw + vec3(0.0, 0.0, 1.0), lightmaps.data[ofs].light_texture_size).rgb - vec3(0.5)) * 2.0;
			lm_light_l1_0 = (sample_lightmap_array(lightmap_textures[ofs], uvw + vec3(0.0, 0.0, 2.0), lightmaps.data[ofs].light_texture_size).rgb - vec3(0.5)) * 2.0;
			lm_light_l1p1 = (sample_lightmap_array(lightmap_textures[ofs], uvw + vec3(0.0, 0.0, 3.0), lightmaps.data[ofs].light_texture_size).rgb - vec3(0.5)) * 2.0;
			...
			ambient_light += sample_lightmap_array(lightmap_textures[ofs], uvw, lightmaps.data[ofs].light_texture_size).rgb * lightmaps.data[ofs].exposure_normalization;
		}
	}
#else
	...
	{ // Directional light.
		...
		if (bool(instances.data[instance_index].flags & INSTANCE_FLAGS_USE_LIGHTMAP)) {
			...
			if (shadowmask_mode != LIGHTMAP_SHADOWMASK_MODE_NONE) {
				...
				shadowmask = sample_lightmap_array(lightmap_textures[MAX_LIGHTMAP_TEXTURES + ofs], uvw, lightmaps.data[ofs].light_texture_size).x;
			}
		}
